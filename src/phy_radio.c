...
static int32_t scanTimerAlarmCallback(phyRadioTaskTimer_t *interface);
static int32_t sendFastTimerCallback(phyRadioFastTaskTimer_t *interface);
...
static int32_t queuePeakOnTxQueue(phyRadioTdma_t* scheduler, phyRadioPacket_t**  pkt);
static int32_t queuePopFromTxQueue(phyRadioTdma_t* scheduler, phyRadioPacket_t**  pkt);
static int32_t queuePutInTxQueue(phyRadioTdma_t* scheduler, phyRadioPacket_t*   packet);
static int32_t queuePutInSlot(phyRadioTdma_t* scheduler, uint8_t slot, phyRadioPacket_t* packet);
...
static int32_t sendDuringCb(phyRadio_t *inst, phyRadioTdma_t* tdma_scheduler, uint8_t slot) {
    ...
    if (tdma_scheduler->packet_delay_time_us < PHY_RADIO_MAX_BLOCK_DELAY_TIME_US) {
        ...
        res = phyRadioTimerStartFastTaskTimer(&inst->fast_task_timer, sendFastTimerCallback, (uint32_t)tdma_scheduler->packet_delay_time_us);
        if (res != PHY_RADIO_TIMER_SUCCESS) {
            LOG_TIMER_ERROR("Timer error %i\n", 5);
...
static int32_t resetTdmaScheduler(phyRadioTdma_t *inst) {
    ...
    return phyRadioSlotHandlerResetSlots(&inst->slot_handler);
}
...
static int32_t initTdmaScheduler(phyRadioTdma_t *inst, const phyRadioTdmaInit_t *init_struct) {
    ...
    int32_t res = STATIC_QUEUE_INIT(&inst->static_queue, inst->items, PHY_RADIO_NUM_ITEMS);
    if (res != STATIC_QUEUE_SUCCESS) {
        return res;
    }
    ...
    // Initialize slot_handler first (without frame_sync - will be set later)
    phyRadioSlotHandlerInit_t slot_init;
    slot_init.phy_radio_inst = inst->phy_radio_inst;
    slot_init.hal_radio_inst = inst->hal_radio_inst;
    slot_init.hal_interface  = inst->hal_interface;
    slot_init.rx_buffer      = init_struct->rx_buffer;

    res = phyRadioSlotHandlerInit(&inst->slot_handler, &slot_init);
    if (res != PHY_RADIO_SLOT_HANDLER_SUCCESS) {
        return res;
    }

    // Now initialize frame_sync with slot_handler pointer
    const phyRadioFrameSyncInit_t frame_sync_init = {
        .slot_handler   = &inst->slot_handler,
        ...
        .active_item    = &inst->active_item,
    };

    res = phyRadioFrameSyncInit(&inst->frame_sync, &frame_sync_init);
    if (res != PHY_RADIO_FRAME_SYNC_SUCCESS) {
        return res;
    }
...
static int32_t processCentral(phyRadio_t *inst) {
    ...
    switch(phyRadioSlotHandlerGetCurSlotType(&tdma_scheduler->slot_handler)) {
...
static int32_t phyRadioManageFrameGuardStart(phyRadio_t *inst) {
    int32_t res = phyRadioSlotHandlerRestoreSlots(&inst->tdma_scheduler.slot_handler);
    if (res != PHY_RADIO_SLOT_HANDLER_SUCCESS) {
        return res;
    }

    // Increment the sync counter once every superframe
    if (phyRadioFrameSyncGetFrameCount(&inst->tdma_scheduler.frame_sync) == 0) {
...
static int32_t phyRadioManageFrameStart(phyRadio_t *inst) {
    ...
    for (int i = 0; i < num_items; i++) {
        ...
        if (rem_in_slot[phy_packet->slot] > packet_time_us) {
            ...
            int32_t res = phyRadioSlotHandlerSetSlotCur(&inst->tdma_scheduler.slot_handler, phy_packet->slot, PHY_RADIO_SLOT_TX);
            if (res != PHY_RADIO_SLOT_HANDLER_SUCCESS) {
                return res;
            }
...
static inline int32_t sendCentral(phyRadio_t *inst, phyRadioPacket_t* packet, bool this_frame) {
    ...
    if (this_frame) {
        ...
        // Set the slot as TX
        int32_t res = phyRadioSlotHandlerSetSlotCur(&inst->tdma_scheduler.slot_handler, packet->slot, PHY_RADIO_SLOT_TX);
        if (res != PHY_RADIO_SLOT_HANDLER_SUCCESS) {
            return res;
        }
...
int32_t phyRadioProcess(phyRadio_t *inst) {
    ...
    res = phyRadioSlotHandlerProcess(&inst->tdma_scheduler.slot_handler);
    if (res != PHY_RADIO_SLOT_HANDLER_SUCCESS) {
        return res;
    }
...
int32_t phyRadioDeInit(phyRadio_t *inst) {
    ...
    if ((res = phyRadioSlotHandlerDeInit(&inst->tdma_scheduler.slot_handler)) != PHY_RADIO_FRAME_SYNC_SUCCESS) {
        LOG("Failed to deinit frame sync\n");
    }
...
int32_t phyRadioSetScanMode(phyRadio_t *inst, uint32_t timeout_ms) {
    ...
    if (timeout_ms > 0) {
        ...
        res = phyRadioTimerStartTaskTimer(&inst->task_timer, scanTimerAlarmCallback, timeout_us);
        if (res != PHY_RADIO_TIMER_SUCCESS) {
            LOG_TIMER_ERROR("Timer error %i\n", 13);
            return PHY_RADIO_TIMER_ERROR;
        }
...
int32_t phyRadioTransitionPeripheralToCentral(phyRadio_t *inst) {
    // The central device's sync slot defaults to IDLE, frame_sync will set it to TX when queueing
    int32_t res = phyRadioSlotHandlerSetSlotMain(&inst->tdma_scheduler.slot_handler, PHY_RADIO_CENTRAL_TX_SLOT, PHY_RADIO_SLOT_IDLE);
    if (res != PHY_RADIO_SLOT_HANDLER_SUCCESS) {
        return res;
    }

    res = phyRadioSlotHandlerSetSlotCur(&inst->tdma_scheduler.slot_handler, PHY_RADIO_CENTRAL_TX_SLOT, PHY_RADIO_SLOT_IDLE);
    if (res != PHY_RADIO_SLOT_HANDLER_SUCCESS) {
        return res;
    }
    ...
    res = phyRadioFrameSyncSetMode(&inst->tdma_scheduler.frame_sync, PHY_RADIO_FRAME_TRANS_TO_CENTRAL);
    if (res != PHY_RADIO_TIMER_SUCCESS) {
...
int32_t phyRadioTransitionCentralToPeripheral(phyRadio_t *inst, uint8_t new_central_addr) {
    ...
    int32_t res = phyRadioSlotHandlerSetSlotMain(&inst->tdma_scheduler.slot_handler, PHY_RADIO_PERIPHERAL_RX_SLOT, PHY_RADIO_SLOT_RX);
    if (res != PHY_RADIO_SLOT_HANDLER_SUCCESS) {
        return res;
    }
    ...
    res = phyRadioFrameSyncSetMode(&inst->tdma_scheduler.frame_sync, PHY_RADIO_FRAME_TRANS_TO_PERIPHERAL);
    if (res != PHY_RADIO_TIMER_SUCCESS) {
        LOG_TIMER_ERROR("Frame sync Error %i\n", res);
        return res;
    }
...
int32_t phyRadioSetCentralMode(phyRadio_t *inst) {
    ...
    // Configure the slot - defaults to IDLE, frame_sync will set to TX when queueing
    res = phyRadioSlotHandlerSetSlotCur(&inst->tdma_scheduler.slot_handler, PHY_RADIO_CENTRAL_TX_SLOT, PHY_RADIO_SLOT_IDLE);
    if (res != PHY_RADIO_SLOT_HANDLER_SUCCESS) {
        return res;
    }

    res = phyRadioSlotHandlerSetSlotMain(&inst->tdma_scheduler.slot_handler, PHY_RADIO_CENTRAL_TX_SLOT, PHY_RADIO_SLOT_IDLE);
    if (res != PHY_RADIO_SLOT_HANDLER_SUCCESS) {
        return res;
    }
...
int32_t phyRadioSetFrameStructure(phyRadio_t *inst, phyRadioFrameConfig_t *frame) {
    ...
    int32_t res = phyRadioFrameSyncSetInterval(&inst->tdma_scheduler.frame_sync, frame->sync_interval);
    if (res != PHY_RADIO_FRAME_SYNC_SUCCESS) {
        return res;
    }
